...
export OCR_LANGUAGES=$(jq --raw-output '.languages // empty' $CONFIG_PATH)
export OCR_NOTIFY_URL=$(jq --raw-output '.notify_url // empty' $CONFIG_PATH)
...
ensure_tempdir() {
    ...
    mkdir -p "${TARGET_DIR}"
    mkdir -p "${OCR_INPUT_DIR:-"/input"}"
    mkdir -p "${OCR_OUTPUT_DIR:-"/output"}"
}

populate_defaults() {
    if [[ ! -f /config/ocr.config ]]; then
        cp /app/ocr.config ${OCR_CONFIG_DIR:-"/config"}/ocr.config
        chown docker:docker ${OCR_CONFIG_DIR:-"/config"}/ocr.config
    fi
}
